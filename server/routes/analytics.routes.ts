...
import { cached, buildCacheKey, TTL, invalidateTenantCache } from '../lib/cache';
...
analyticsRouter.get('/rfm/distribution', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const cacheKey = buildCacheKey('rfm:dist', tenantId);

    const result = await cached(cacheKey, TTL.LONG, async () => {
...
analyticsRouter.get('/rfm/matrix', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = req.tenant!.id;
    const cacheKey = buildCacheKey('rfm:matrix', tenantId);

    const result = await cached(cacheKey, TTL.LONG, async () => {
      return getRfmMatrix(tenantId);
    });
...
analyticsRouter.get('/rfm/summary', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = req.tenant!.id;
    const cacheKey = buildCacheKey('rfm:summary', tenantId);
...
analyticsRouter.get('/cohorts/monthly', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const cacheKey = buildCacheKey('cohorts:monthly', tenantId, monthsBack);

    const result = await cached(cacheKey, TTL.DAY, async () => {
...
analyticsRouter.get('/cohorts/retention', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { months = '6' } = req.query;
    const monthsBack = parseInt(months as string, 10);
    const cacheKey = buildCacheKey('cohorts:retention', tenantId, monthsBack);
...
analyticsRouter.get('/revenue/trend', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const cacheKey = buildCacheKey('revenue:trend', tenantId, daysBack);
...
analyticsRouter.get('/revenue/summary', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const tenantId = req.tenant!.id;
    const cacheKey = buildCacheKey('revenue:summary', tenantId);

    const result = await cached(cacheKey, TTL.SHORT, async () => {
...
analyticsRouter.get('/customers/top', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const cacheKey = buildCacheKey('customers:top', tenantId, metric as string, limitNum);
